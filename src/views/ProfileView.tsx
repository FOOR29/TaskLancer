        ...
        <div className="min-h-[calc(100vh-95px)] bg-(--bg-1) py-8">
            <div className="max-w-4xl mx-auto px-6">
                ...
                <div className="bg-(--bg-2) border border-(--border-1) rounded-2xl p-8 mb-6">
                    <h1 className="text-(--text-1) text-2xl font-bold mb-2">Edit Profile</h1>
                    <p className="text-(--text-2)">Manage your personal information</p>
                </div>
                ...
                <div className="bg-(--bg-2) border border-(--border-1) rounded-2xl p-6 mb-6">
                    <div className="flex items-center gap-6">
                        ...
                        {/* Upload Info */}
                        <div>
                            <p className="text-(--text-1) font-semibold mb-1">Upload new photo</p>
                            <p className="text-(--text-2) text-sm">At least 800×800 px recommended.</p>
                            <p className="text-(--text-2) text-sm">JPG or PNG is allowed</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-(--bg-2) border border-(--border-1) rounded-2xl p-6 mb-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-(--text-1) text-lg font-semibold">Personal Info</h2>
                        <button 
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div>
                            <p className="text-(--text-2) text-sm mb-1">Full Name</p>
                            <p className="text-(--text-1) font-medium">{profile?.name || 'Not set'}</p>
                        </div>
                        <div>
                            <p className="text-(--text-2) text-sm mb-1">Email</p>
                            <p className="text-(--text-1) font-medium">{profile?.email || 'Not set'}</p>
                        </div>
                        <div>
                            <p className="text-(--text-2) text-sm mb-1">Phone</p>
                            <p className="text-(--text-1) font-medium">{profile?.phone || 'Not set'}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-(--bg-2) border border-(--border-1) rounded-2xl p-6 mb-6">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-(--text-1) text-lg font-semibold">Location</h2>
                        <button 
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex-1 flex items-center gap-3 px-4 py-3 bg-(--bg-3) border border-(--border-1) rounded-xl">
                            ...
                            <span className="text-(--text-1)">{profile?.location || 'Not set'}</span>
                        </div>
                        <button 
                            onClick={() => setEditingSection('location')}
                ...
                <div className="bg-(--bg-2) border border-(--border-1) rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-(--text-1) text-lg font-semibold">Bio</h2>
                        <button 
